...
func (f *EnvAWSFingerprint) Fingerprint(request *FingerprintRequest, response *FingerprintResponse) error {
	...
	if specs := f.lookupCPU(ec2meta); specs != nil {
		...
		f.logger.Debug("lookup ec2 cpu", "cores", specs.cores, "MHz", log.Fmt("%.0f", specs.mhz))
...
func (f *EnvAWSFingerprint) lookupCPU(ec2meta *ec2metadata.EC2Metadata) *ec2specs {
	...
	if err != nil {
		f.logger.Warn("failed to read EC2 metadata region", "error", err)
		return nil
	}

	info := f.cpuFromTable(region, instanceType)
	if info == nil {
		return nil
	}
...
// cpuFromTable uses the generated AWS EC2 CPU performance lookup table to return
// the documented performance characteristics of the CPU for the given instance
// type in the given region.
func (*EnvAWSFingerprint) cpuFromTable(region, instance string) *ec2specs {
	instances, exists := ec2ProcSpeedTable[region]
	if !exists {
		return nil
	}

	data, exists := instances[instance]
	if !exists {
		return nil
	}
